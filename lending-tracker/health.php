...
try {
    ...
    $expected = ['users', 'lending_accounts', 'deposits', 'payment_logs', 'settings'];
    foreach ($expected as $t) {
        try {
            db()->query('SELECT 1 FROM `' . $t . '` LIMIT 1');
            $tables[$t] = 'OK';
        } catch (Throwable $e) {
            $tables[$t] = 'FALTA/ERROR';
        }
    }
} catch (Throwable $e) {
        ...
        <?php if ($dbOk): ?>
            ...
            <ul>
                <?php foreach ($tables as $name => $status): ?>
                    <li><strong><?php echo htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?>:</strong> <?php echo htmlspecialchars($status, ENT_QUOTES, 'UTF-8'); ?></li>
                <?php endforeach; ?>
            </ul>
        <?php endif; ?>
