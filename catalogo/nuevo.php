...
$errors = [];
$data   = ['codigo_tango'=>'','nombre'=>'','descripcion'=>'','precio'=>'','stock'=>'0','categoria'=>''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $pdo->prepare("
            INSERT INTO productos (codigo_tango,nombre,descripcion,precio,stock,categoria)
            VALUES (?,?,?,?,?,?)
        ")->execute([
            $data['codigo_tango'], $data['nombre'], $data['descripcion'],
            (float)$data['precio'], $data['stock'], $data['categoria']
        ]);
