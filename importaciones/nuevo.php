...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        flash('Importación creada.');
        redirect('/importaciones/ver.php?id=' . $iid);
    }
}
...
<div class="max-w-2xl">
  <a href="<?= BASE_URL ?>/importaciones/" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-4">
  ...
  <form method="POST" class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4">
    ...
    <div class="grid sm:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <input type="text" name="proveedor" value="<?= esc($_POST['proveedor'] ?? '') ?>" required
          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Origen (país)</label>
        <input type="text" name="origen" value="<?= esc($_POST['origen'] ?? '') ?>" placeholder="China, India…"
          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Familia de productos</label>
        <input type="text" name="familia_productos" value="<?= esc($_POST['familia_productos'] ?? '') ?>"
          placeholder="Elevadores, Neumáticos…"
          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">N° de proforma</label>
        <input type="text" name="numero_proforma" value="<?= esc($_POST['numero_proforma'] ?? '') ?>"
          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Monto FOB (USD)</label>
        <input type="number" name="monto_fob" value="<?= esc($_POST['monto_fob'] ?? '') ?>"
      ...
      <div>
        ...
        <select name="estado"
          ...
          <option value="<?= $v ?>" <?= ($_POST['estado'] ?? 'pendiente') === $v ? 'selected' : '' ?>><?= $l ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      ...
      <div class="sm:col-span-2">
        ...
        <select name="forwarder_id"
          ...
          <option value="<?= $fw['id'] ?>" <?= ($_POST['forwarder_id'] ?? '') == $fw['id'] ? 'selected' : '' ?>>
            <?= esc($fw['nombre']) ?>
          </option>
          <?php endforeach; ?>
        </select>
      ...
      <div class="sm:col-span-2">
        ...
        <textarea name="observaciones" rows="3"
          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"><?= esc($_POST['observaciones'] ?? '') ?></textarea>
      </div>

    </div>

    <div class="flex justify-end gap-3 pt-2">
      <a href="<?= BASE_URL ?>/importaciones/" class="text-sm text-gray-500 px-4 py-2">Cancelar</a>
